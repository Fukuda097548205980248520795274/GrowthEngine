	...
	NavMesh* navMesh, StageEditorNavMeshController* navMeshController, bool& isDirty, bool canExtrude, bool canBridge)
{
	...
	if (currentMode_ == EditorMode::ObjectPlacement)
	{
		...
		if (selectedIndex_ >= 0 && selectedIndex_ < static_cast<int>(placementList.size()))
		{
			const auto& target = placementList[selectedIndex_];

			// イベントトリガーの生成ファイル編集UIを表示する条件
			if (target.category == EditCategory::Object &&
				target.subType == static_cast<int>(StageObject::StageObjectTag::StaticEventTrigger) &&
				target.eventType == 1)
			{
				...
				if (ImGui::TreeNode(("生成ファイル編集: " + fileName + ".json").c_str()))
				{
					...
					if (ImGui::Button("オブジェクト追加"))
					{
						...
						selectedPreviewIndex_ = cachedPreviewData_.size() - 1;
					}
					ImGui::SameLine();
...
void StageEditorUI::LoadStageDataNames()
{
	stageDataNames_.clear();
...
/// @param fileName 
void StageEditorUI::LoadPreviewData(const std::string& fileName)
{
...
void StageEditorUI::SavePreviewData()
{
