...
void GuardEffect::Initialize(PrefabInstanceTube* instanceModel, const Vector3& position, const Vector3& rotation)
{
	...
	instanceModel_->param_.size.radiusTop = 0.5f;
	instanceModel_->param_.size.radiusBottom = 0.5f;
}
...
void GuardEffect::Update()
{
	...
	instanceModel_->param_.size.radiusBottom = Lerp(0.5f, 1.0f, t);
	
	if (t >= 0.0f && t <= 0.5f)
	{
		float t2 = t / 0.5f;
		instanceModel_->param_.size.radiusTop = Lerp(0.5f, 1.0f, t2);
	}
