#include "ComboTreeEditorHistory.h"
...
/// @param links 
/// @param currentId 
void ComboTreeEditorHistory::SaveHistory(const std::vector<ComboEditorNode>& nodes, const std::vector<ComboEditorLink>& links, int currentId)
{
	// 現在の状態をスナップショットとして保存
...
/// @param editor 
void ComboTreeEditorHistory::Undo(ComboTreeEditor& editor)
{
	...
	currentSnapshot.nodes = editor.nodes_;
	currentSnapshot.links = editor.links_;
	currentSnapshot.currentId = editor.currentId_;
	currentSnapshot.selectedNodes = currentSelectedNodes;
	currentSnapshot.selectedLinks = currentSelectedLinks;
...
/// @param editor 
void ComboTreeEditorHistory::Redo(ComboTreeEditor& editor)
{
...
/// @brief 履歴をクリアする
void ComboTreeEditorHistory::Clear()
{
	undoHistory_.clear();
	redoHistory_.clear();
}