...
void NavMeshMove::Update()
{
    ...
    if (len > kMinMoveDistance)
    {
        ...
        if (!owner_->IsPlayer() && !owner_->IsInAttackSequence())
        {
            ...
            // 前回のベクトルから目標ベクトルへ補間する
            float lerpFactor = 0.15f;
            currentMoveDirection_.x = currentMoveDirection_.x + (moveDirection.x - currentMoveDirection_.x) * lerpFactor;
            currentMoveDirection_.y = currentMoveDirection_.y + (moveDirection.y - currentMoveDirection_.y) * lerpFactor;
