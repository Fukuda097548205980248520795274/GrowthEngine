#include "Render3DAnimationModelData.h"
...
/// @param modelStore 
/// @param device 
void Engine::Render3DAnimationModelData::Initialize(ModelStore* modelStore, TextureStore* textureStore, AnimationStore* animationStore, LightStore* lightStore, ID3D12Device* device, Log* log)
{
	// nullptrチェック
	assert(modelStore);
	assert(textureStore);
...
/// @brief 更新処理
void Engine::Render3DAnimationModelData::Update()
{
	// 描画を記録する
	isPreDrew_ = isDrew_;
	isDrew_ = false;
...
/// @brief リセット
void Engine::Render3DAnimationModelData::Reset()
{
	// モデルデータを取得する
	const ModelData& modelData = modelStore_->GetModelData(hModel_);
...
/// @param pso 
/// @param textureStore 
void Engine::Render3DAnimationModelData::Register(Camera3DStore* cameraStore, SkyboxStore* skyboxStore, ID3D12GraphicsCommandList* commandList, BasePSOModel* pso)
{
	// 読み込まれていないときは処理しない
	if (!isLoad_)return;
...
/// @param commandList 
/// @param pso 
void Engine::Render3DAnimationModelData::Register(const Matrix4x4& viewProjection, ID3D12GraphicsCommandList* commandList, BasePSOShadowMap* pso)
{
	// 読み込まれていないときは処理しない
	if (!isLoad_)return;
...
/// @brief デバッグ用パラメータ
void Engine::Render3DAnimationModelData::DebugParameter()
{
#ifdef _DEVELOPMENT
...
/// @param ray 
/// @param pickList 
void Engine::Render3DAnimationModelData::DebugRayPicker(const Collision3D::Ray& ray, std::vector<std::pair<float, DebugData::DebugGuizmoData*>>& pickList)
{
	// 選択初期化
