...
namespace Engine
{
	...
	class Render3DRingData : public Render3DBaseData
	{
	public:
		...
		/// @param name 
		/// @param hTexture 
		Render3DRingData(const std::string& name, Render3DHandle hRender3D) : Render3DBaseData(name, hRender3D) { type_ = Render3D::Type::Ring; }
	...
	private:
		...
		/// @brief パラメータ
		std::unique_ptr<Render3D::Ring::Param> param_ = nullptr;
