...
namespace Engine
{
	...
	class Collision3DBaseData
	{
	public:

		/// @brief コンストラクタ
		/// @param func 
		Collision3DBaseData(std::function<void()> func, const std::string& name, Collision3D::Type type, Collision3DHandle hCollision)
			: func_(func), name_(name), type_(type), hCollision_(hCollision) {
		}

		/// @brief 仮想デストラクタ
		virtual ~Collision3DBaseData() = default;
		...
		/// @return 
		std::list<std::unique_ptr<BaseCollision3DInstance>>& GetInstanceTable() { return instanceTable_; }
	...
	protected:
		...
		// インスタンステーブル
		std::list<std::unique_ptr<BaseCollision3DInstance>> instanceTable_;
	};
}