...
/// @param hAudio 
/// @param audioStore 
void Engine::BgmData::Initialize(AudioStore* audioStore, SoundParameter* parameter, AudioHandle hAudio)
{
	// nullptrチェック
	assert(audioStore);
	assert(parameter);
	...
	// サウンドデータ
	param_ = std::make_unique<BgmParam>();
	param_->volume = 0.5f;
	param_->pitch = 1.0f;
...
/// @brief 再生されているかどうか
/// @return 
bool Engine::BgmData::IsPlay() const { return audioStore_->IsAudioPlay(hPlay_); }
