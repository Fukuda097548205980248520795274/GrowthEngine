#include "ComputePSOMotionBlur.h"
#include "Log/Log.h"
...
/// @param compiler 
/// @param log 
void Engine::ComputePSOMotionBlur::Initialize(ID3D12Device* device, ShaderCompiler* compiler, Log* log)
{
	// シェーダのバイナリデータを取得する
	computeShaderBlob_ = compiler->Compile(L"./Assets/Shader/PostEffect/MotionBlur.CS.hlsl", L"cs_6_0");
	...
	rootParameter[2].DescriptorTable.pDescriptorRanges = writeTextureDescriptor;
	rootParameter[2].DescriptorTable.NumDescriptorRanges = _countof(writeTextureDescriptor);

	// 定数バッファ RootConstant b0
	rootParameter[3].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;
	rootParameter[3].ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL;
	rootParameter[3].Constants.ShaderRegister = 0;
	rootParameter[3].Constants.RegisterSpace = 0;
