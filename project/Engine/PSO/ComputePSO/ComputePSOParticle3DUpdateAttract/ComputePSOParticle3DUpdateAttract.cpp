#include "ComputePSOParticle3DUpdateAttract.h"
#include "Log/Log.h"
...
/// @param compiler 
/// @param log 
void Engine::ComputePSOParticle3DUpdateAttract::Initialize(ID3D12Device* device, ShaderCompiler* compiler, Log* log)
{
	// シェーダのバイナリデータを取得する
	computeShaderBlob_ = compiler->Compile(L"./Assets/Shader/Particle3D/Update/Particle3DUpdateAttract.CS.hlsl", L"cs_6_0");
	...
	rootParameter[3].DescriptorTable.pDescriptorRanges = freeListIndexDescriptor;
	rootParameter[3].DescriptorTable.NumDescriptorRanges = _countof(freeListIndexDescriptor);
