...
/// @param compiler 
/// @param log 
void Engine::ComputePSOTAA::Initialize(ID3D12Device* device, ShaderCompiler* compiler, Log* log)
{
	// シェーダのバイナリデータを取得する
	computeShaderBlob_ = compiler->Compile(L"./Assets/Shader/PostEffect/TAA.CS.hlsl", L"cs_6_0");
	...
	D3D12_DESCRIPTOR_RANGE prevTextureDescriptor[1];
	prevTextureDescriptor[0].BaseShaderRegister = 1;
	prevTextureDescriptor[0].RegisterSpace = 0;
	prevTextureDescriptor[0].NumDescriptors = 1;
	prevTextureDescriptor[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
	prevTextureDescriptor[0].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;

	D3D12_DESCRIPTOR_RANGE velocityTextureDescriptor[1];
	velocityTextureDescriptor[0].BaseShaderRegister = 2;
	...
	rootParameter[1].DescriptorTable.pDescriptorRanges = prevTextureDescriptor;
	rootParameter[1].DescriptorTable.NumDescriptorRanges = _countof(prevTextureDescriptor);

	// SRV DescriptorTable t2
	rootParameter[2].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
	rootParameter[2].ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL;
	rootParameter[2].DescriptorTable.pDescriptorRanges = velocityTextureDescriptor;
	rootParameter[2].DescriptorTable.NumDescriptorRanges = _countof(velocityTextureDescriptor);
