#include "ComputePSOVelocityDilation.h"
#include "Log/Log.h"
...
/// @param compiler 
/// @param log 
void Engine::ComputePSOVelocityDilation::Initialize(ID3D12Device* device, ShaderCompiler* compiler, Log* log)
{
	// シェーダのバイナリデータを取得する
	computeShaderBlob_ = compiler->Compile(L"./Assets/Shader/PostEffect/VelocityDilation.CS.hlsl", L"cs_6_0");
	...
	velocityTextureDescriptor[0].BaseShaderRegister = 0;
	...
	rootParameter[0].DescriptorTable.pDescriptorRanges = velocityTextureDescriptor;
	rootParameter[0].DescriptorTable.NumDescriptorRanges = _countof(velocityTextureDescriptor);
