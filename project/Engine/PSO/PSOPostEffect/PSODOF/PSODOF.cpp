...
/// @param log 
void Engine::PSODOF::Initialize(ID3D12Device* device, ShaderCompiler* compiler, IDxcBlob* vertexShaderBlob, Log* log)
{
	// nullptrチェック
	assert(device);
	assert(compiler);
	...
	pixelShaderBlob_ = compiler->Compile(L"./Assets/Shader/PostEffect/DOF.PS.hlsl", L"ps_6_0");
	...
	// t1 ブラー用テクスチャ
	D3D12_DESCRIPTOR_RANGE blurTextureDescriptor[1];
	blurTextureDescriptor[0].BaseShaderRegister = 1;
	blurTextureDescriptor[0].RegisterSpace = 0;
	blurTextureDescriptor[0].NumDescriptors = 1;
	blurTextureDescriptor[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
	blurTextureDescriptor[0].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;

	// t2 深度テクスチャ
	D3D12_DESCRIPTOR_RANGE depthTextureDescriptor[1];
	depthTextureDescriptor[0].BaseShaderRegister = 2;
	...
	// DescriptorTable ブラー用テクスチャ
	rootParameter[1].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
	rootParameter[1].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;
	rootParameter[1].DescriptorTable.pDescriptorRanges = blurTextureDescriptor;
	rootParameter[1].DescriptorTable.NumDescriptorRanges = _countof(blurTextureDescriptor);

	// DescriptorTable 深度テクスチャ
	rootParameter[2].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
	rootParameter[2].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;
	rootParameter[2].DescriptorTable.pDescriptorRanges = depthTextureDescriptor;
	rootParameter[2].DescriptorTable.NumDescriptorRanges = _countof(depthTextureDescriptor);
