...
/// @param log 
void Engine::PSODissolve::Initialize(ID3D12Device* device, ShaderCompiler* compiler, IDxcBlob* vertexShaderBlob, Log* log)
{
	// nullptrチェック
	assert(device);
	assert(compiler);
	...
	pixelShaderBlob_ = compiler->Compile(L"./Assets/Shader/PostEffect/Dissolve.PS.hlsl", L"ps_6_0");
	...
	// マスク用のディスクリプタレンジ
	D3D12_DESCRIPTOR_RANGE maskDescriptor[1];
	maskDescriptor[0].BaseShaderRegister = 1;
	maskDescriptor[0].RegisterSpace = 0;
	maskDescriptor[0].NumDescriptors = 1;
	maskDescriptor[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
	maskDescriptor[0].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;
	...
	rootParameter[2].DescriptorTable.pDescriptorRanges = maskDescriptor;
	rootParameter[2].DescriptorTable.NumDescriptorRanges = _countof(maskDescriptor);
