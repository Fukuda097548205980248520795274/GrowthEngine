...
/// @param compiler 
/// @param log 
void Engine::PSOTrail::Initialize(ID3D12Device* device, ShaderCompiler* compiler, Log* log)
{
	// nullptrチェック
	assert(device);
	assert(compiler);
	...
	vertexShaderBlob_ = compiler->Compile(L"./Assets/Shader/Trail/Trail.VS.hlsl", L"vs_6_0");
	assert(vertexShaderBlob_ != nullptr);

	// ピクセルシェーダのバイナリデータを取得する
	pixelShaderBlob_ = compiler->Compile(L"./Assets/Shader/Trail/Trail.PS.hlsl", L"ps_6_0");
