...
/// @param compiler 
/// @param log 
void Engine::PSOTriangle::Initialize(ID3D12Device* device, ShaderCompiler* compiler, Log* log)
{
	// nullptrチェック
	assert(device);
	assert(compiler);
	...
	vertexShaderBlob_ = compiler->Compile(L"./Assets/Shader/Triangle/Triangle.VS.hlsl", L"vs_6_0");
	assert(vertexShaderBlob_ != nullptr);

	// ピクセルシェーダのバイナリデータを取得する
	pixelShaderBlob_ = compiler->Compile(L"./Assets/Shader/Triangle/Triangle.PS.hlsl", L"ps_6_0");
...
/// @brief コマンドリストに登録する
/// @param commandList 
void Engine::PSOTriangle::Register(ID3D12GraphicsCommandList* commandList)
{
	// nullptrチェック
	assert(commandList);
