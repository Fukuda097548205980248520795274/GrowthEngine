...
/// @param log 
void Engine::PSOFullscreen::Initialize(ID3D12Device* device, IDxcBlob* vertexShaderBlob, IDxcBlob* pixelShaderBlob, Log* log)
{
	// nullptrチェック
	assert(device);
	...
	rootParameter[0].DescriptorTable.pDescriptorRanges = descriptorTexture;
	rootParameter[0].DescriptorTable.NumDescriptorRanges = _countof(descriptorTexture);
