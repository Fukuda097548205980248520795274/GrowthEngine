...
void Engine::PSOPrefabSprite::Initialize(ID3D12Device* device, IDxcBlob* vertexShaderBlob, IDxcBlob* pixelShaderBlob, Log* log)
{
	...
	// SRV t0 パラメータ
	D3D12_DESCRIPTOR_RANGE descriptorParam[1];
	descriptorParam[0].BaseShaderRegister = 0;
	descriptorParam[0].RegisterSpace = 0;
	descriptorParam[0].NumDescriptors = 1;
	descriptorParam[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
	descriptorParam[0].OffsetInDescriptorsFromTableStart = D3D12_DESCRIPTOR_RANGE_OFFSET_APPEND;
	...
	// DescriptorTable VertexShader パラメータ
	rootParameter[0].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
	rootParameter[0].ShaderVisibility = D3D12_SHADER_VISIBILITY_VERTEX;
	rootParameter[0].DescriptorTable.pDescriptorRanges = descriptorParam;
	rootParameter[0].DescriptorTable.NumDescriptorRanges = _countof(descriptorParam);
