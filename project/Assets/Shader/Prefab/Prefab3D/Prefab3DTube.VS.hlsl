...
VertexShaderOutput main(VertexShaderInput input, uint instanceID : SV_InstanceID)
{
    ...
    // UVのY座標 (奥=0.0、手前=1.0) を使って、この頂点に適用すべき半径を補間
    float currentRadius = lerp(gPrimitive[instanceID].topRadius, gPrimitive[instanceID].bottomRadius, input.texcoord.y);

    // 頂点位置の変形 (XY平面を半径で、Z軸を長さでスケール)
    float4 localPos;
    localPos.x = input.position.x * currentRadius;
    localPos.y = input.position.y * currentRadius;
    localPos.z = input.position.z * gPrimitive[instanceID].height;
    localPos.w = 1.0f;
    
    // テーパー(円錐台)に対応した法線の再計算
    // Z軸向きシリンダーの position.xy は cos(phi), sin(phi) と同値であることを利用
    float dr = gPrimitive[instanceID].bottomRadius - gPrimitive[instanceID].topRadius;
    float3 localNormal = normalize(float3(gPrimitive[instanceID].height * input.position.x, gPrimitive[instanceID].height * input.position.y, dr));
