...
void main( uint3 DTid : SV_DispatchThreadID )
{
    ...
    for (uint countIndex = 0; countIndex < gEmitter[emitterIndex].count; ++countIndex)
    {
        ...
        if (0 <= freeListIndex && freeListIndex <= gMaxNum.particleNum)
        {
            ...
            // 放出位置
            float3 emitDirection = normalize((generator.Generate3d() * 2.0f) - float3(1.0f, 1.0f, 1.0f));
            float3 emitPos = gEmitter[emitterIndex].position.xyz + emitDirection * gEmitRadius.radius1;
